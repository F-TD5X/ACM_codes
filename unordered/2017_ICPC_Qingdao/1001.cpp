...
int main()
{
	...
	scanf("%d",&T);
	while(T--){
		BigFloat x1,x2,x3,x4,y1,y2,y3,y4;
		BigFloat tr,r,x,y;
		cin >> x1>>y1>>x2>>y2>>x3>>y3>>x>>y;
		BigFloat two,zero;
		two="2.0";
		zero="0.0";
		x4=((y2-y1)*(y3*y3-y1*y1+x3*x3-x1*x1)-(y3-y1)*(y2*y2-y1*y1+x2*x2-x1*x1))/(two*(x3-x1)*(y2-y1)-two*((x2-x1)*(y3-y1)));    
    	y4=((x2-x1)*(x3*x3-x1*x1+y3*y3-y1*y1)-(x3-x1)*(x2*x2-x1*x1+y2*y2-y1*y1))/(two*(y3-y1)*(x2-x1)-two*((y2-y1)*(x3-x1)));    
		tr = (x-x4)*(x-x4)+(y-y4)*(y-y4);
		r= (x1-x4)*(x1-x4)+(y1-y4)*(y1-y4);
		if(tr<=r)
			cout <<"Rejected"<<endl;
		else
			cout <<"Accepted"<<endl;
